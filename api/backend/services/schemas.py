...
class EnglishKnowledgeItem(BaseModel):
    ...
    category: Literal[
        ...
        "Grammar & Usage",
        "Test Strategy",
    ] = Field(..., description="Nhóm kỹ năng theo SAT English")
...
class SATEnglishMeta(BaseModel):
    ...
    text_type: Optional[
        Literal["science", "history", "literature", "social_science"]
    ] = Field(None, description="Loại văn bản (nếu xác định được)")

    difficulty_band: Optional[Literal["easy", "medium", "hard"]] = Field(
...
class EnglishSolutionStep(BaseModel):
    ...
    description: str = Field(..., description="Nội dung bước suy luận")
    derivation: str = Field(..., description="Vì sao bước này hợp logic")
...
class EnglishAnswerChoiceAnalysis(BaseModel):
    ...
    summary: str = Field(..., description="Tóm tắt ngắn nội dung đáp án")
    ...
    ] = Field(None, description="Nếu sai: loại lỗi SAT chuẩn")
...
class EnglishSolutionPath(BaseModel):
    ...
    approach_type: Literal[
        "keyword_first", "logic_first", "elimination_first", "exam_trick"
    ] = Field(..., description="Loại chiến lược giải")
    title: str = Field(..., description="Tên ngắn gọn của hướng giải")
    ...
    pros: Optional[str] = Field(None, description="Ưu điểm của hướng giải")
    cons: Optional[str] = Field(None, description="Nhược điểm của hướng giải")
    best_when: Optional[str] = Field(
        None, description="Khi nào nên dùng hướng này trong SAT"
    )
